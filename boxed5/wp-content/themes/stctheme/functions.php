...
<!--  STYLING TO SAVE TO SITE FROM WORDPRESS CUSTOMIZER (PREVIOUSLY THEME CUSTOMIZER)  -->
	 <style type="text/css">
	  ...
	  <?php if( get_theme_mod('wpt_custom_css') != '' ) {
	     ...
	     .top_bar_top_container { padding: <?php echo get_theme_mod( 'stc_upper_top_padding' ), '%'; ?>; }
	     .nav-bar-out-wrapper { height: <?php echo get_theme_mod( 'stc_topbar_height' ), 'px'; ?>; }
	     .top_bar_top_container { background-color: <?php echo get_theme_mod( 'stc_upper_top_color' ) ?>; }
	     ...
	     .top_bar_top_container { height: <?php echo get_theme_mod( 'stc_upper_top_height' ), 'px'; ?>; }
		 ...
		 .top_bar_top_container { display: none; }
		 <?php } // end if ?>
...
// Register admin dashbaord contact form
function createAjaxContactForm() {
	echo '
	<form class="dash_contact_wrapper">
		<div class="dash_contact_container">
			<div class="dash_contact_class" id="name_contact_dash">
			<label class="dash_inline_1" for="name">Name</label>
			<input class="dash_inline_2" type="text" name="name" id="name" size="30" tabindex="1" />
			</div>
			<div class="dash_contact_class" id="dash_contact_email">
			<label class="dash_inline_1" for="email">Email address</label>
			<input class="dash_inline_2" type="text" name="email" id="email" size="30" tabindex="2" />
			</div>
			<div class="dash_contact_message" id="dash_contact_message">
			<label for="message">Message</label>
			<textarea  name="message" id="message" tabindex="3"></textarea>
			</div>
			<div id="dash_contact_send">
			<input type="hidden" name="action" value="contactform_action" />
			'.wp_nonce_field('ajax_contactform', '_acf_nonce', true, false).'
			<button class="dash_contact_submit" type="button" id="contactbutton">Submit</button>
			</div>
		</div>
	</form>
...
function myplugin_add_dashboard_widgets() {
    wp_add_dashboard_widget('contact_widget_dashboard', 'Contact STC Devloper', 'createAjaxContactForm');
}

add_action('wp_dashboard_setup', 'myplugin_add_dashboard_widgets' );

// Add css to admin area
add_action('admin_head', 'my_contact_widget');

function my_contact_widget() {
  echo '<style>
    .dash_contact_class {
	display: block;
overflow: hidden;
...
.dash_contact_message label {
	display: block;
width: 100%;
text-align: center;
}
.dash_contact_message textarea {
	width: 100%;
height: 8em;
}
.dash_contact_submit {
	width: 100%;
background-color: rgb(46, 114, 204);
